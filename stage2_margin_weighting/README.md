...
## One-round correction (documented per task spec section 8)

**Original full run (3 seeds, quick_mode=false) verdict: `not_supported`
(1/5 checks passed).** Margin-weighted was significantly WORSE than
baseline on far_rmse, 15-step rollout RMSE, and every closed-loop metric
(violation_rate, max_violation, cumulative_objective, tracking_rmse, all
p<0.02 Wilcoxon), and near_rmse only improved in 1 of 3 seeds. This was
not just "no improvement" -- it was a consistent regression.

**Root cause identified**: `generate_data.py`'s trajectory rollout used to
hard-clip position to the boundary and zero velocity on contact
("inelastic wall stop") when generating training data. This introduced an
artificial DISCONTINUITY concentrated exactly in the near-constraint
region that is not part of the task's specified (smooth) nonlinearity.
Up-weighting samples dominated by an unlearnable jump does not help a
smooth MLP -- it just steals training capacity from the rest of the state
space, which is consistent with far_rmse and rollout error both getting
much worse under margin-weighting.

**Correction applied** (this round only, per the "at most one correction
round, keep original results" rule): removed the hard wall-clip entirely;
the plant now evolves purely per the specified smooth nonlinear dynamics.
The existing quadratic drag term (-0.15*v*|v|) already provides physically
realistic self-limiting behavior, so trajectories don't blow up. Some
transitions may now legitimately have a negative margin (position briefly
beyond the nominal bound) -- left in the data as realistic rather than
clipped away. `NEAR_FRAC_INIT` was raised from 0.0 to 0.2 to compensate
for the lost near-boundary dwell time that the wall-bounce used to provide
(only changes initial-position sampling density, not noise or near-region
treatment -- still passes the "same noise mechanism everywhere" check).
**This has not been re-run**; `run_experiment.py` now auto-archives the
previous run's `data/`, `checkpoints/`, `results/` into a timestamped
`archive_pre_correction_<timestamp>/` folder before a fresh run, so the
original `not_supported` result is preserved regardless.
